...
public sealed class TicketService(ApplicationDbContext dbContext, ICurrentUser currentUser) : ITicketService
{
    public async Task<PagedResult<TicketSummaryDto>> GetTicketsAsync(TicketQuery query, CancellationToken cancellationToken = default)
    {
        ...
        if (!string.IsNullOrWhiteSpace(query.Search))
        {
            ...
            tickets = tickets.Where(ticket => ticket.Number.Contains(search) ||
    ...
    public async Task<TicketDetailDto> GetTicketAsync(string number, CancellationToken cancellationToken = default) =>
    ...
    public async Task<TicketDetailDto> CreateTicketAsync(CreateTicketRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var number = await GenerateNumberAsync(cancellationToken);
    ...
    public async Task UpdateStatusAsync(string number, UpdateTicketStatusRequest request, CancellationToken cancellationToken = default)
    {
        var ticket = await GetAccessibleTicketAsync(number, cancellationToken, includeDetails: false);
        if (currentUser.Role == UserRole.Customer && (request.Status != TicketStatus.Closed || ticket.Status != TicketStatus.Resolved))
            throw new ForbiddenException("Customers may only close their own resolved tickets.");
        if (currentUser.Role is not (UserRole.Admin or UserRole.SupportAgent or UserRole.Customer))
    ...
    public async Task UpdatePriorityAsync(string number, UpdateTicketPriorityRequest request, CancellationToken cancellationToken = default)
    {
    ...
    public async Task UpdateAssignmentAsync(string number, AssignTicketRequest request, CancellationToken cancellationToken = default)
    {
        EnsureRole(UserRole.Admin);
        var ticket = await GetAccessibleTicketAsync(number, cancellationToken, includeDetails: false);
        User? agent = null;
    ...
    public async Task AddCommentAsync(string number, AddCommentRequest request, CancellationToken cancellationToken = default)
    {
        var ticket = await GetAccessibleTicketAsync(number, cancellationToken, includeDetails: false);
    ...
    public async Task LogTimeAsync(string number, LogTimeRequest request, CancellationToken cancellationToken = default)
    {
        EnsureRole(UserRole.SupportAgent);
        var ticket = await GetAccessibleTicketAsync(number, cancellationToken, includeDetails: false);
        if (ticket.Status == TicketStatus.Closed)
    ...
    public async Task<IReadOnlyCollection<TicketActivityDto>> GetTimelineAsync(string number, CancellationToken cancellationToken = default)
    {
    ...
    private IQueryable<Ticket> Scope(IQueryable<Ticket> tickets)
    {
        ...
        return currentUser.Role switch
        {
            ...
            UserRole.SupportAgent => tickets.Where(ticket => ticket.AssignedSupportId == currentUser.UserId),
            UserRole.Customer => tickets.Where(ticket => ticket.CustomerId == currentUser.UserId),
            _ => tickets.Where(_ => false)
        };
    }

    private async Task<Ticket> GetAccessibleTicketAsync(string number, CancellationToken cancellationToken, bool includeDetails = true)
    {
    ...
    private async Task<string> GenerateNumberAsync(CancellationToken cancellationToken)
    {
        ...
        {
            var number = $"TKT-{DateTime.UtcNow:yyyyMMdd}-{Guid.NewGuid():N}"[..21].ToUpperInvariant();
            if (!await dbContext.Tickets.AnyAsync(ticket => ticket.Number == number, cancellationToken))
                return number;
        }
        throw new ConflictException("Could not generate a unique ticket number. Please retry.");
    }
