    ...
    IPasswordHasher<User> passwordHasher) : IUserService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (role.HasValue)
        {
            var domainRole = role.Value switch
            {
                UserRoleFilter.Admin => UserRole.Admin,
                UserRoleFilter.SupportAgent => UserRole.SupportAgent,
                UserRoleFilter.Customer => UserRole.Customer,
                _ => throw new ValidationException("A valid role is required.")
            };
            users = users.Where(user => user.Role == domainRole);
        }
        if (activeOnly)
    ...
    private void EnsureAdmin()
    {
        ...
        if (currentUser.Role != UserRoleNames.Admin)
